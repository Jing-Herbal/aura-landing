...
const JING = (() => {
  ...
  const PRODUCTS = {
    "aura": {
      ...
      variants: {
        "month": { id: "month", label: "Single Box", desc: "120 g · 30 × 4 g sticks · 4 week ritual", price: 588, compare: null },
        "subscribe": { id: "subscribe", label: "Subscribe · 4 Weeks", desc: "One box delivered monthly · pause anytime", price: 498, compare: 588 }
      }
    }
  };
  ...
  function renderCartUI(cart) {
    ...
    if (shipNote) {
      if (total >= FREE_SHIP_THRESHOLD) {
        shipNote.textContent = "You have free Hong Kong shipping ✓";
      } else if (total > 0) {
        shipNote.textContent = `${fmt(FREE_SHIP_THRESHOLD - total)} away from free HK shipping`;
      } else {
        shipNote.textContent = `Free HK shipping over ${fmt(FREE_SHIP_THRESHOLD)}`;
      }
    }
